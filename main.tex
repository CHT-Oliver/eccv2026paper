...
\begin{abstract}
  Long-video VLMs often exhibit peaked attention, where a few tokens absorb most attention mass and degrade long-range reasoning. We show that this behavior is linked to anisotropic Q/K statistics in the temporal mRoPE subspace: when phase diversity over valid positions is limited, cross-frequency coupling remains, producing sharp second-order spectra and unstable coverage. We propose \textbf{SPECTRA}, a training-free inference-time module for mRoPE-based video LLMs. SPECTRA performs PCA on temporal Q/K channels to estimate per-head effective rank, maps this signal to layer-wise and head-wise gates, and applies gated Gaussian interpolation only on video-temporal channels. This intervention lifts small eigenvalues, smooths the spectrum, and broadens attention coverage without retraining or architecture changes. Across ActivityNet-Captions, Charades-STA, Video-MME, LVBench, and MVBench, SPECTRA consistently improves MRoPE, YaRN, and Dynamic NTK baselines, with gains up to 2.9 points overall and stronger improvements on long-video splits.
