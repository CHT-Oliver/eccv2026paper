...
\begin{abstract}
  Long-video understanding with mRoPE-based VLMs often suffers from peaked attention, where a small set of tokens dominates attention mass and weakens long-range reasoning. We revisit this failure from a second-order perspective and show that isotropization is governed by two coupled conditions: intra-subspace uniformity within each 2D rotary block and inter-subspace decoupling across blocks. The latter is controlled by discrete phase-difference cancellation; insufficient phase coverage leaves residual cross-frequency coupling and yields sharp spectra. Building on this analysis, we propose \textbf{SPECTRA}, a training-free and plug-and-play inference module that targets temporal mRoPE channels only. SPECTRA estimates per-head spectral degeneration via PCA-based effective rank, assigns intervention strength with layer-wise and head-wise gates, and applies gated Gaussian interpolation to smooth the Q/K spectrum. This mechanism increases spectral flatness, mitigates attention spikes, and expands long-range coverage without retraining or architecture changes. Experiments on long-video understanding and temporal grounding benchmarks demonstrate consistent gains over strong mRoPE scaling baselines.
  \keywords{Long-video VLMs \and Rotary Positional Encoding \and Attention Anisotropy \and Training-free Inference Adaptation}
\end{abstract}
